...
- **One file per page** (`page.tsx` route). Filename = route path after the portal, dashes for slashes, `[id]` -> `id`. Examples: `gov/incidents.md`, `gov/incidents-id.md`, `gov/fleet-trips-id.md`, `logistics/operator.md`. Portal root page = `home.md` (the site root `/` is `public/landing.md`; `/account`, `/status`, `/forbidden` live in `shared/`). Link other memory files with `[[portal/file]]`.
