...
export function useFieldHomeData(): FieldHomeData {
  ...
  const myReportsCount = useMemo(() => {
    const serverMine = (reports.data ?? []).filter((r) => r.reporter_id === me.user_id).length;
    const localUnsynced = pendingOps.length;
    return serverMine + localUnsynced;
  }, [reports.data, me.user_id, pendingOps]);
  ...
  const corridorReports = useMemo(
    () => (reports.data ?? []).filter((r) => snapToCorridor(r.location.latitude, r.location.longitude).isWithinCorridor),
    [reports.data],
  );
  const corridorIncidents = useMemo(() => {
    const ids = new Set(corridorReports.map((r) => r.id));
    return (incidents.data ?? []).filter((inc) => ids.has(inc.primary_report_id));
  }, [incidents.data, corridorReports]);
  ...
  const corridorHealth = useMemo<CorridorHealthSummary>(() => {
    const feats = edges.data?.features ?? [];
    let blocked = 0;
    let restricted = 0;
    let caution = 0;
    let open = 0;
  ...
  const latestReport = useMemo<FieldReportPreview | null>(() => {
    ...
    const myServerReports = (reports.data ?? [])
    ...
    return null;
  }, [pendingOps, reports.data, me.user_id]);
  ...
  const weatherNotice = useMemo<string | null>(() => {
    const zones = (riskZones.data?.zones ?? []).filter((z) => z.riskLevel === "HIGH" || z.riskLevel === "SEVERE");
